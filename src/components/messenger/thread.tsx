...
import { ArrowLeft, Ban, MoreVertical, Phone, Video } from "lucide-react";
...
export function Thread({ onBack }: { onBack: () => void }) {
  ...
  if (!selectedId || !conv) {
    return (
      <div className="hidden h-full flex-col items-center justify-center bg-bg md:flex">
        <p className="font-display text-2xl font-medium tracking-tight text-fg">Pick a conversation</p>
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-bg">
      ...
      <div ref={scroller} className="vesper-scroll min-h-0 flex-1 space-y-4 overflow-y-auto px-3 py-4 md:px-6">
        {grouped.length === 0 ? (
