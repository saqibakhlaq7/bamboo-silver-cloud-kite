...
function RemoteAudio({ stream }: { stream: MediaStream | null }) {
  ...
  if (!stream) return null;
  return (
    <audio ref={ref} autoPlay playsInline className="pointer-events-none fixed left-0 top-0 h-px w-px opacity-0" />
  );
}
...
export function CallOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-bg text-fg">
      ...
      {!call.video ? <RemoteAudio stream={call.remoteStream} /> : null}
      ...
      <div className="relative z-10 flex items-center justify-center gap-3 px-6 pb-10">
        {incoming ? (
          <>
            <Button size="lg" variant="danger" onClick={rejectCall} className="rounded-full px-7">
              <PhoneOff className="size-4" />
              Decline
            </Button>
            <Button size="lg" onClick={() => void acceptCall()} className="rounded-full px-7">
