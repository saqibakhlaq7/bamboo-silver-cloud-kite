...
import { sanitizeAvatar, sanitizeLocalMedia } from "./safe-media";
    ...
    (hits: DirectoryHit[]) => {
      ...
      setPeople((prev) => {
        ...
        for (const hit of hits) {
          ...
          const photo = sanitizeAvatar(hit.photoData) || old?.photoData || null;
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        let snap: InboxSnapshot;
        try {
          snap = (await syncInbox({
            data: {
              ackIds: acks,
        ...
        for (const ev of snap.events) {
          try {
            ...
            const media = sanitizeLocalMedia((ev.payload.media as LocalMedia | null) ?? null);
            ...
            await bumpConversation(db, {
              ...
              lastAt: createdAt,
              unreadDelta: !existing && !viewing ? 1 : 0,
              resetUnread: viewing,
            });
            if (ev.payload.from) {
              mergePeople([
                {
                  ...
                  photoData: sanitizeAvatar(ev.payload.from.photoData),
                  lastSeen: Date.now(),
                  online: true,
                },
              ]);
            ...
            /* keep the event on the server until we persist it */
          }
        }
      } catch {
